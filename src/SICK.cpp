...
float getOD2000Value(String JsonString)
{
    ...
    return sickSensor.getByte2IntValue(DataArr, 130, -6);
}
...
float getWTM10LValue(String JsonString)
{
    byte DataArr[4];
    JsonDocument rdoc;
    Serial.println(JsonString);
    ...
    return sickSensor.getByte2IntValue(DataArr,0,0,2);
}
...
void TaskSICKSensor(void *pvParameter)
{
    ...
    for (;;)
    {
        Serial.println("OD2000 Distance: " + String(getOD2000Value(OD2000)) + " mm");
        gyroArr = getMPB10Value(MPB10_1);
        Serial.println("Temperature: " + String(gyroArr[0]));
        Serial.println("Z axis 3: " + String(gyroArr[1]));
        Serial.println("Y axis 2: " + String(gyroArr[2]));
        Serial.println("X axis 1: " + String(gyroArr[3]));
        Serial.println("WTM10L Distance: " + String(getWTM10LValue(WTM10L)) + " mm");
